...
LEDS	EQU		0x0ff1			; ??LED（如NumLock)状态的存入地址
...
[INSTRSET "i486p"]				; “想要使用486指令”的叙述（之后就可以使用386以后的LGDT，EAX，CR0等关键字）
...
; 所有剩下的
		...
		MOV		ECX,0
		MOV		CL,BYTE [CYLS]
		IMUL	ECX,512*18*2/4	; 从柱面数变换为字节数的1/4
		SUB		ECX,512/4		; 减去IPL
		CALL	memcpy

; 必须由asmhead来完成的工作，至今全部完毕
;	以后就交由bootpack来完成
